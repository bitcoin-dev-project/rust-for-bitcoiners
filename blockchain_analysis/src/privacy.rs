use std::{collections::HashSet, ops::Add, result};
...
pub fn txs_sort_unique_input_addrs(client: &Client, block_height: u64) -> Result<Vec<(Transaction, u32)>, bitcoincore_rpc::Error> {
    ...
    for tx in txs.iter() {
        ...
        for input in tx.input.iter() {
            let prev_tx = client.get_raw_transaction(&input.previous_output.txid, Some(&block_hash))?;
            let prev_tx_output = &prev_tx.output[input.previous_output.vout as usize];
            let prev_tx_script = &prev_tx_output.script_pubkey;
            let input_address = Address::from_script(prev_tx_script, Network::Bitcoin).expect("Failed to parse address from scriptPubKey");
